...
import 'package:user/app/modules/user/domain/use_cases/delete_user_use_case.dart';
import 'package:user/app/modules/user/presenter/stores/delete_user/delete_user_state.dart';
import 'package:user/app/modules/user/presenter/stores/delete_user/delete_user_store.dart';
...
void main() {
  late MockDeleteUserUseCase useCase;
  late DeleteUserStore store;
  late UserEntity user;

  setUp(() {
    useCase = MockDeleteUserUseCase();
    store = DeleteUserStore(useCase);
  ...
  test(
    ...
    () async {
      final statusChanged = MockCallable<DeleteUserState>();

      when(() => useCase.call(user: any(named: 'user'))).thenAnswer(
        (_) async => const Right(true),
      );

      mobx.reaction<DeleteUserState>(
        (_) => store.state,
        (newValue) => statusChanged(newValue),
      );

      await store.delete(user: user);

      verifyInOrder([
        () => statusChanged(LoadingDeleteUserState()),
        () => statusChanged(SuccessDeleteUserState()),
      ]);
    },
  );
  ...
  test(
    ...
    () async {
      ...
      verifyInOrder([
        () => statusChanged(LoadingDeleteUserState()),
        () => statusChanged(const FailureDeleteUserState(error: '')),
      ]);
    },
  );
}
